...
struct get_actions_subcommand {
   ...
   get_actions_subcommand(CLI::App* app) {

      auto cmd = app->add_subcommand("actions", localized("Retrieve all actions with specific account name referenced in authorization or receiver"), false);
      cmd->add_option("account_name", account_name, localized("name of account to query on"))->required();
      cmd->add_option("pos", pos_seq, localized("sequence number of action for this account, -1 for last"));
      cmd->add_option("offset", offset, localized("get actions [pos,pos+offset] for positive offset or [pos-offset,pos) for negative offset"));
      cmd->add_flag("--json,-j", printjson, localized("print full json"));
      cmd->add_flag("--full", fullact, localized("don't truncate action output"));
      cmd->add_flag("--pretty", prettyact, localized("pretty print full action json "));
      cmd->add_flag("--console", printconsole, localized("print console output generated by action "));
      cmd->set_callback([this] {
...
static void print_voters_table(const fc::variants& table) {

    std::cout << "+---------------+------------------------+-----------+" << std::endl;
    std::cout << "| Account       | Weight                 | Last Vote |" << std::endl;
    std::cout << "+---------------+------------------------+-----------+" << std::endl;
    if ( !table.empty() ) {
        for ( auto& row : table ) {
            const auto& obj = row.get_object();

            printf("| %-13.13s | %-22.4f | %9li |\n",
                  obj["account"].as_string().c_str(),
                  obj["weight"].as_double(),
                  obj["last_vote"].as_int64());
        }
    } else {
            std::cout << "|               | -                      | -         |" << std::endl;
    }
...
int main( int argc, char** argv ) {
   ...
   // get voters
   auto getVoters = get_voters_subcommand(get);
   ...
   // get actions
   auto getActions = get_actions_subcommand(get);
